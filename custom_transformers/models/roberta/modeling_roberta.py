...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.classifier_input_length, config.hidden_size)
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.dropout = nn.Dropout(classifier_dropout)
        self.out_proj = nn.Linear(config.hidden_size, config.num_labels)
        self.use_cls = config.use_cls if hasattr(config, "use_cls") else True

    def forward(self, features, **kwargs):
        if self.use_cls:
            x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
        else: # Flatten from dim = 1 to the end. The hidden size must be equal to: sequence_length x hidden_size
